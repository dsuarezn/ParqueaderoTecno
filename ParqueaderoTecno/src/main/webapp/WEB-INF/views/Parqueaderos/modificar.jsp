...
<div class="body">
	...
	<form class="form-horizontal" role="form" action="${modificarUrl}" method="POST">
		...
		<div class="form-group elementoFormParqueadero">
			...
			<div class="col-sm-3">
				<input type="text" class="form-control" id="tipoParqueadero" name="tipoParqueadero" value="${parqueadero.tipoParqueadero}" placeholder="<spring:message code="modificarParqueadero.labels.placeholder.tipo"/>"	${parqueadero.esCrear != true ? 'readonly' : ' '} required autofocus>
			</div>
		</div>
		<div class="form-group elementoFormParqueadero">
			<label class="control-label col-sm-2" for="espacios"><spring:message code="modificarParqueadero.labels.espacios" /></label>
			<div class="col-sm-3">
				<input type="number" min=1 class="form-control" id="espacios" name="espacios" value="${parqueadero.espacios !=0 ? parqueadero.espacios : '' }"	placeholder="<spring:message code="modificarParqueadero.labels.placeholder.espacios"/>" required autofocus>
			</div>
		</div>
		<div class="form-group elementoFormParqueadero">
		    <label class="control-label col-sm-2" for="estado"><spring:message code="modificarParqueadero.labels.habilitado"/></label>
		    <div class="col-sm-3 text-left"> 
		      <input type="checkbox" class="form-control" ${parqueadero.estado == true ? 'checked' : ' '} id="estado" name="estado">
		    </div>
		  </div>
		<div class="form-group elementoFormParqueadero">
